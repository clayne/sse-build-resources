...
namespace stl
{
    ...
    public:
        using map_type = M;
        using map_value_type = typename M::value_type;

        using value_pointer_type = map_value_type*;
        ...
        vectormap(const std::vector<std::pair<K, V>>& a_rhs)
        {
            for (auto& e : a_rhs)
            {
                try_emplace(e.first, e.second);
            }
        }

        vectormap(std::vector<std::pair<K, V>>&& a_rhs)
        {
            for (auto& e : a_rhs)
            {
