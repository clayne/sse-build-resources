...
class TaskDelegateStatic : public TaskDelegate
{
public:
	virtual void Dispose() override{};
};
...
public:
	...
	template <class = is_pointer<element_type>, class = is_base_type<TaskFunctor>>
	void AddTask(const TaskFunctor::func_t& a_func)
	{
		IScopedLock _(m_lock);
		m_queue.emplace(new TaskFunctor(a_func));
	}
...
protected:
	...
	inline bool TaskQueueEmpty() const
	{
		IScopedLock _(m_lock);
