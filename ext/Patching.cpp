...
namespace Patching
{
	...
	{
		DWORD oldProtect;
		_assert(VirtualProtect(reinterpret_cast<void*>(addr), len, PAGE_EXECUTE_READWRITE, &oldProtect));
		memcpy(reinterpret_cast<void*>(addr), data, len);
		_assert(VirtualProtect(reinterpret_cast<void*>(addr), len, oldProtect, &oldProtect));
	}
