...
namespace hash
{
	...
	__forceinline constexpr char toupper_ascii(char a_char) noexcept
	{
		return (a_char > 0x60 && a_char < 0x7B) ? a_char - 0x20 : a_char;
	}

	__forceinline constexpr std::uint8_t toupper_ascii_l1s(std::uint8_t a_char) noexcept
	{
	...
	__forceinline constexpr char toupper(char a_char) noexcept
	{
		return static_cast<char>(toupper_ascii_l1s(static_cast<std::uint8_t>(a_char)));
	}

	__forceinline wchar_t towupper(wchar_t a_char)
	{
	...
	namespace fnv1
	{
		...
		__forceinline static std::size_t _append_hash_bytes_fnv1a(
			std::size_t a_hash,
		...
		__forceinline static std::size_t _append_hash_bytes_fnv1(
			std::size_t a_hash,
			const std::uint8_t* const a_in,
			std::size_t a_size)
		{
